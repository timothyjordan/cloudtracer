...
export async function scanHosting(domain: string, platform: Platform): Promise<HostingInfo> {
  const [v4, v6] = await Promise.all([
    platform.dns.resolveA(domain),
    platform.dns.resolveAaaa(domain),
  ]);
  ...
  if (v4.length > 0) {
    const asnInfo = await lookupAsn(v4[0], platform);
    if (asnInfo) {
      result.asn = asnInfo;
...
async function lookupAsn(
  ...
  try {
    ...
    const records = await platform.dns.resolveTxt(`${reversed}.origin.asn.cymru.com`);
    ...
    // Look up ASN name
    const nameRecords = await platform.dns.resolveTxt(`AS${asnNumber}.asn.cymru.com`);
